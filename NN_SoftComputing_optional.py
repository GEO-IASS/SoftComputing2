...
if __name__ == '__main__':
    seed(1)
    f=open("/Users/manasgaur/Downloads/double_dog_dare_data",'r')
    temp=[]
    for line in f.readlines():
        temp.append((line.split('\n')[0].replace('\'','')))
    print  "Number of tuples in the data (data is still of string type)",len(temp)
    data=[]
    for i in range(len(temp)):
        ...
        for t in range(len(terms)):
            ...
            x.append(tmp)
        data.append(x)
    print "Numerical format converted data", len(data)
    newdata = data
    dataset=[]
    ''' Preparing the data'''
    ...
    for fld in dataSplit:
        ...
        trainingNetwork(netK, trainset, 1, 100, numoutputs)
