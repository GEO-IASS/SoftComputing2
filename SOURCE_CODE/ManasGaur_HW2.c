               ...
               gradient_accumulator += (network->output_layer[j].gradient *
                                   ...
                                   //  network->hidden_layer[k].gradient = network->hidden_layer[k].output *
                                     //       (1.0 - network->hidden_layer[k].output) *
                                       //     (network->hidden_layer[j].output) *  //INDEXING ERROR
...
void update(network_ptr network, double learning_rate)
...
{ int j,k;
  ...
  for (j=0; j<network->output_neuron_count; j++)
      { for (k=0; k<network->hidden_neuron_count; k++)
        ...
        network->output_layer[j].bupdates += (learning_rate * network->output_layer[j].gradient);
...
{ int c;
  int random_index;
  int random_class; // Variable to store the class
  double Y1, Y2;
...
{30.65,	-8.30}};
  ...
  /* Set up neural network */
 	 // malloc_network(2,4, 2, &my_network); /* Two input nodes, four hidden nodes, two output node */
 	
  malloc_network(2, 8, 2, &my_network); /* Two input nodes, eight hidden nodes, two output node */


//	  malloc_network(2, 2, 2, &my_network); /* Two input nodes, two hidden nodes, two output node */
   ...
   */
   ...
   int epoch=0,j,k;
	float Error=80; //ANy random value for the epoch to enter the for loop
	float error=43; //Any random value less than Error
	while((Error -error)>0.0000000001){ 
	Error=error;
	error=0;
	for (j=0; j<my_network->output_neuron_count; j++)
      { for (k=0; k<my_network->hidden_neuron_count; k++)
             my_network->second_layer_wupdates[j*my_network->hidden_neuron_count+k]=0;}
	for (j=0; j<my_network->hidden_neuron_count; j++)
      { for (k=0; k<my_network->input_node_count; k++)
             my_network->first_layer_wupdates[j*my_network->input_node_count+k]=0;}
	for (j=0; j<my_network->hidden_neuron_count; j++){
          my_network->hidden_layer[j].bupdates =0;}
	for (j=0; j<my_network->output_neuron_count; j++){
          my_network->output_layer[j].bupdates =0;}
	 
   for (random_class=1; random_class<4; random_class++)
      {
        
        if (random_class == 1){
        		for(random_index=0;random_index<16;random_index++)
           { X[0][0] = class_01[random_index][0];
             X[0][1] = class_01[random_index][1];
				compute_network_outputs(X[0], my_network);                    /* feed inputs forward  */
        		compute_network_gradients(D[random_class-1], my_network);
				update(my_network, 0.05);      /* compute weights and adds the weights for each epoch in a new variable      */
           }}
        if (random_class == 2){
        		for(random_index=0;random_index<13;random_index++)
           { X[0][0] = class_02[random_index][0];
             X[0][1] = class_02[random_index][1];
				compute_network_outputs(X[0], my_network);                   /* feed inputs forward  */
        		compute_network_gradients(D[random_class-1], my_network);
				update(my_network,0.05);      /* compute weights and adds the weights for each epoch in a new variable      */
           }}
        if (random_class == 3){
				for(random_index=0;random_index<7;random_index++)
           { X[0][0] = class_03[random_index][0];
             X[0][1] = class_03[random_index][1];
				compute_network_outputs(X[0], my_network);                     /* feed inputs forward  */
        		compute_network_gradients(D[random_class-1], my_network);
				update(my_network,0.05);      /* compute weights and adds the weights for each epoch in a new variable       */
           }}
		...
		for (random_class=1; random_class<4; random_class++)
      {
        ...
        if (random_class == 1){
           ...
           { X[0][0] = class_01[random_index][0];
             X[0][1] = class_01[random_index][1];
				compute_network_outputs(X[0], my_network);                    /* feed inputs forward  */
				error+=((D[0][0]-my_network->output_layer[0].output)*(D[0][0]-my_network->output_layer[0].output)+(D[0][1]-my_network->output_layer[1].output)*(D[0][1]-my_network->output_layer[1].output));      /* compute gradients       */
           }}
        if (random_class == 2){
        		for(random_index=0;random_index<13;random_index++)
           { X[0][0] = class_02[random_index][0];
             X[0][1] = class_02[random_index][1];
				compute_network_outputs(X[0], my_network);                   /* feed inputs forward  */ 
				error+=((D[1][0]-my_network->output_layer[0].output)*(D[1][0]-my_network->output_layer[0].output)+(D[1][1]-my_network->output_layer[1].output)*(D[1][1]-my_network->output_layer[1].output));     /* compute gradients       */
           }}
        if (random_class == 3){
				for(random_index=0;random_index<7;random_index++)
           { X[0][0] = class_03[random_index][0];
             X[0][1] = class_03[random_index][1];
				...
				error+=((D[2][0]-my_network->output_layer[0].output)*(D[2][0]-my_network->output_layer[0].output)+(D[2][1]-my_network->output_layer[1].output)*(D[2][1]-my_network->output_layer[1].output));      /* compute gradients       */
           }}
        
               
      }
		epoch++;        
		error=error/64;                   /* adjust weights       */
	//printf("Error %f\n",error);
...
printf("  Error= %f \n",error);
   ...
   printf("-----------------------------------------\n");
    ...
    print_network_parameters(my_network);
		printf("  Number of epoches = %d \n",epoch);//Prints the number of epoches to reach zero error
